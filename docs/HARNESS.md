...
- **禁止**僅因 API 已呼叫或 Agent 文字宣稱成功即視為完成。
- **實作（v1.1+）**：任務 Agent 回合結束後，Harness 另起**獨立 MCP 唯讀驗證回合**（`core/pipeline/verification.py`），要求輸出 JSON（`verified` + `checks`），寫入 `actual_after.harness_verification`；未通過則 `status: failed`、`verification: failed`。
- 仍依驗證 LLM 正確使用 MCP；除錯可用 `--skip-verification` 或 `expected.skip_harness_verification: true`。
