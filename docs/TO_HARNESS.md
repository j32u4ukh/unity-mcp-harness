...
- [x] **4.1** `core/pipeline/runner.py`：`on_task_start` / `on_task_end` + 本 repo `UnityMCPRunner`
- [x] **4.2** `build_workflow._run_single_task` 掛 hook + `save_task_list`
- [x] **4.3** `run_build.py` 依 `task_list` 取下一個非 `completed` 任務
...
- [x] **6.1** `core/pipeline/tool_adapter.py`：Read/Write 慣例 + `operations_executed`
- [x] **6.2** 任務欄位 `harness.pre_read` / `harness.post_read`
- [x] **6.3** 失敗 → `status: failed`、`verification: failed`
- [x] **6.4** 冪等跳過 → `verification: skipped_by_idempotent`

### 階段 7 — 執行期動態任務注入

- [x] **7.1** `store.inject_subtask()`
- [x] **7.2** `[HARNESS_INJECT:...]` 或 tool 觸發
- [x] **7.3** `get_next_runnable_task` 尊重 `priority` / 插入序
...
- [x] **9.1** `test_pipeline_store.py`、`test_bootstrap.py`
- [x] **9.2** `--dry-run`：normalize + bootstrap 或讀既有 task_list
- [x] **9.3**（可選）`--json` 含 `verification`
- [x] **9.4** `test_plan_normalize_writeback.py`
...
- [x] **10.5** CLI 更名 `unity-mcp-harness`
